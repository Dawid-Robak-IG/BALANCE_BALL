...
void ball_set_speed(int16_t *speed_x, int16_t *speed_y) {
	...
	lcd_change_ball_color((uint16_t) (abs(*speed_x)),
			(uint16_t) (abs(*speed_y)));
}
...
bool check_collision(Rectangle rec, int next_x, int next_y) {
	...
	if ((next_x < rec.x + rec.width + player.r) && (next_x > rec.x - player.r)
			&& (next_y > rec.y - player.r)
			&& (next_y < rec.y + rec.height + player.r)
			&& (rec.color == GREEN)) {
		screen_id++;
...
bool check_inside_screen(int next_x, int next_y) {
	if (next_x - player.r
			<= 0|| next_x + player.r >= LCD_WIDTH || next_y - player.r <= 0 || next_y + player.r >= LCD_HEIGHT) {
		return false;
	}
	return true;
}
